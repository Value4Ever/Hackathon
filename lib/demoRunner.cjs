...
function buildScript(options) {
  ...
  return [
    ...
    { step: 18, time_offset_ms: 31000, scene: '10:08 — SPC Chart Featured',
      title: 'SPC + Western Electric Rules — visual proof',
      description: 'Top critical patient: Abigail Rogers. p̄=96%, 28 appts. Rule 1 fires (point > UCL). Rule 2 (9-in-a-row) + Rule 3 (6 trending up) confirm shift. Risk score 100%.',
      action: () => {
        const spc = analyzePatient('CP-00022');
        return {
          patient: 'CP-00022 Abigail Rogers',
          n: spc.n, p_bar_pct: (spc.p_bar*100).toFixed(1),
          ucl_pct: (spc.ucl*100).toFixed(0), lcl_pct: (spc.lcl*100).toFixed(0),
          we_rules_count: spc.we_rules_triggered.length,
          rules_fired: spc.we_rules_triggered.slice(0, 3).map(f => 'Rule ' + f.rule)
        };
      },
